...
public class SDLActivity extends Activity {
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (hasFocus) {
           ...
           // CRAWL HACK: Bring back the keyboard on resume
           if (mScreenKeyboardShown) {
               showTextInput(0, 0, 0, 0);
           }
            ...
            switch(action) {
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (pointerCount == 2) {
                        ...
                        if (historySize > 1) {
                            float scroll0 = event.getHistoricalY(0, 1) - event.getHistoricalY(0, 0);
                            float scroll1 = event.getHistoricalY(1, 1) - event.getHistoricalY(1, 0);
                            if ((scroll0 * scroll1) > 0) { // Both are positive or negative
                                SDLActivity.onNativeMouse(1, MotionEvent.ACTION_SCROLL, 0, scroll0*0.25f);
                            }
                        }
                        return true;
                    }
...
class DummyEdit extends View implements View.OnKeyListener {
    ...
    // CRAWL HACK: Rebind hardware buttons
    @Override
    public boolean onKeyPreIme (int keyCode, KeyEvent event) {
        // Stop SDL from hiding the keyboard when pressing back
        // and use the button as an alternative escape key
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            KeyEvent escapeEvent = new KeyEvent(event.getAction(), KeyEvent.KEYCODE_ESCAPE);
            return super.dispatchKeyEvent(escapeEvent);
        }
        // Zoom in with volume up
        if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            KeyEvent control = new KeyEvent(event.getDownTime(), event.getEventTime(), event.getAction(),
                    KeyEvent.KEYCODE_CTRL_LEFT, event.getRepeatCount(),
                    KeyEvent.META_CTRL_ON | KeyEvent.META_CTRL_LEFT_ON);
            KeyEvent equals = new KeyEvent(event.getDownTime(), event.getEventTime(), event.getAction(),
                    KeyEvent.KEYCODE_EQUALS, event.getRepeatCount(),
                    KeyEvent.META_CTRL_ON | KeyEvent.META_CTRL_LEFT_ON);
            return super.dispatchKeyEvent(control) && super.dispatchKeyEvent(equals);
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
            ...
            KeyEvent minus = new KeyEvent(event.getDownTime(), event.getEventTime(), event.getAction(),
                    KeyEvent.KEYCODE_MINUS, event.getRepeatCount(),
                    KeyEvent.META_CTRL_ON | KeyEvent.META_CTRL_LEFT_ON);
            return super.dispatchKeyEvent(control) && super.dispatchKeyEvent(minus);
        }
