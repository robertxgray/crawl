...
static void _nowrap_eol_cprintf_touchui(const char *format, ...)
{
    ...
    switch (TOUCH_UI_STATE)
    {
        case TOUCH_S_NULL:
            ...
            break;
        case TOUCH_V_TITL2:
            cprintf("%s%s", get_species_abbrev(you.species),
...
#ifdef USE_TILE_LOCAL
    if (tiles.is_using_small_layout())
        MP_Bar.vdraw(6, 23, you.magic_points, you.max_magic_points);
    else
#endif
...
#ifdef USE_TILE_LOCAL
    if (tiles.is_using_small_layout())
        HP_Bar.vdraw(2, 23, you.hp, you.hp_max);
    else
#endif
